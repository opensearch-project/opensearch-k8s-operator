...
func (r *ScalerReconciler) excludeNode(currentStatus opsterv1.ComponentStatus, currentSts appsv1.StatefulSet, nodePoolGroupName string) error {
	...
	//r.recorder.Event(r.instance, "Normal", "Scaler", fmt.Sprintf("Start sacle %s/%s from %s to %s", r.instance.Namespace, r.instance.Name, *currentSts.Spec.Replicas, *currentSts.Spec.Replicas-1))
	...
	if err := r.Get(r.ctx, client.ObjectKey{Name: targetService.Name, Namespace: namespace}, &existingService); err != nil {
		...
		if err != nil {
			if !errors.IsAlreadyExists(err) {
				...
				r.recorder.Event(r.instance, "Warning", "Operator", "Cannot create Headless service -  Requeue - Fix the problem you have on main Opensearch Headless Service ")
				return *targetService, false, err
			}
		}
...
func (r *ScalerReconciler) DeleteNodePortService(service corev1.Service) {
	...
	if err != nil {
		...
		r.recorder.Event(r.instance, "Warning", "Operator", "Cannot delete service - Requeue - Fix the problem you have on main Opensearch Headless Service ")
	}
}
...
func (r *ScalerReconciler) removeStatefulSet(sts appsv1.StatefulSet) (*ctrl.Result, error) {
	...
	r.recorder.Event(r.instance, "Normal", "Scaler", "Created os client for scaling ")
