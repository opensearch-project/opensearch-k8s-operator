...
func (r *SearchTemplateReconciler) Reconcile() (result ctrl.Result, err error) {
	...
	if r.instance.Status.SearchTemplateName == existingSearchTemplate.SearchTemplate.SearchTemplateName && cmp.Equal(*newSearchTemplate, existingSearchTemplate.SearchTemplate, cmpopts.EquateEmpty()) {
		r.logger.V(1).Info(fmt.Sprintf("template %s is in sync", r.instance.Spec.SearchTemplateName))
	...
	err = services.UpdateSearchTemplate(r.ctx, r.osClient, request, &existingSearchTemplate.SequenceNumber, &existingSearchTemplate.PrimaryTerm, existingSearchTemplate.SearchTemplate.SearchTemplateName)
	if err != nil {
		shortReason := "failed to update search templatetemplate template with Opensearch API"
		reason = fmt.Sprintf("%s: %s", shortReason, err.Error())
		r.logger.Error(err, shortReason)
		r.recorder.Event(r.instance, "Warning", opensearchAPIError, shortReason)
		return ctrl.Result{
...
func (r *SearchTemplateReconciler) Delete() error {
	...
	exist, err := services.SearchTemplateExists(r.ctx, r.osClient, templateName)
	if err != nil {
