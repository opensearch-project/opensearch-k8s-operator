...
var _ = Describe("Builders", func() {
	...
	When("Reconcile Job config set", func() {
		...
		It("should Snapshot Repoconfig Update Job set config as jobConfig set", func() {
			clusterObject := ClusterDescWithJobConfig()
			result := NewSnapshotRepoconfigUpdateJob(&clusterObject, "snapshotrepoconfig", "foobar", "", nil, nil)
