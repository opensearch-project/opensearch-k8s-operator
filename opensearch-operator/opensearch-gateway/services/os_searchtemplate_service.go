...
func GetSearchTemplate(ctx context.Context, service *OsClusterClient, policyName string) (*responses.GetSearchTemplateResponse, error) {
	...
	snapshotPolicyResponse := responses.SearchTemplateResponse{}
...
func UpdateSearchTemplate(ctx context.Context, service *OsClusterClient, searchtemplate requests.SearchTemplate, seqno, primterm *int, policyName string) error {
	spec := opensearchutil.NewJSONReader(searchtemplate)
	resp, err := service.UpdateSearchTemplateConfig(ctx, policyName, *seqno, *primterm, spec)
	if err != nil {
		return err
	}
	defer helpers.SafeClose(resp.Body)
