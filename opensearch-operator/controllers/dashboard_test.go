...
import (
	"context"
	"fmt"
	helpers "opensearch.opster.io/pkg/helpers"
	"time"
		...
		It("should create all Opensearch-dashboard resources", func() {
			...
			Eventually(func() bool {
				...
				if OpensearchCluster.Spec.Dashboards.Enable {
					...
					if err := helpers.K8sClient.Get(context.Background(), client.ObjectKey{Namespace: ClusterName, Name: ClusterName + "-dashboards-config"}, &cm); err != nil {
						return false
					}
					if err := helpers.K8sClient.Get(context.Background(), client.ObjectKey{Namespace: ClusterName, Name: OpensearchCluster.Spec.General.ServiceName + "-dashboards"}, &service); err != nil {
						return false
					}
				}
				return true
