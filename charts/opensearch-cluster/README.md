...
| `cluster.security.tls.http.enableHotReload` | bool | `false` | Enable hot reloading of TLS certificates so nodes pick up renewed certificates without a restart. Requires OpenSearch >= 2.19.1. Must be set explicitly: omitting it from the CR defaults to enabled on OpenSearch 3.x+. Set to true/false to override; leave unset (~) to defer to the operator default. |
...
| `cluster.security.tls.transport.enableHotReload` | bool | `false` | Enable hot reloading of TLS certificates so nodes pick up renewed certificates without a restart. Requires OpenSearch >= 2.19.1. Must be set explicitly: omitting it from the CR defaults to enabled on OpenSearch 3.x+. Set to true/false to override; leave unset (~) to defer to the operator default. |
