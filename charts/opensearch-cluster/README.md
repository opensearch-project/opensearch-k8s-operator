...
| `cluster.security.tls.http.enabled` | string | `nil` | Controls if TLS should be enabled for HTTP. If false, TLS is explicitly disabled. If not set (default), TLS is enabled when http configuration is provided. If true, TLS is explicitly enabled and http configuration must be provided. |
...
| `cluster.security.tls.http.enableHotReload` | string | `nil` | Enable hot reloading of TLS certificates so nodes pick up renewed certificates without a restart. Requires OpenSearch >= 2.19.1. Unset (~) defers to the operator: enabled on OpenSearch 3.x+, off below. Set true/false to override. |
| `cluster.security.tls.transport.enabled` | string | `nil` | Controls if TLS should be enabled for transport. If false, TLS is explicitly disabled. If not set (default), TLS is enabled when transport configuration is provided. If true, TLS is explicitly enabled and transport configuration must be provided. |
...
| `cluster.security.tls.transport.enableHotReload` | string | `nil` | Enable hot reloading of TLS certificates so nodes pick up renewed certificates without a restart. Requires OpenSearch >= 2.19.1. Unset (~) defers to the operator: enabled on OpenSearch 3.x+, off below. Set true/false to override. |
...
Opensearch-cluster Helm Chart version: `3.3.2`
