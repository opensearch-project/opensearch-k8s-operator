...
| `cluster.general.rollingRestart.healthGatePolicy` | string | `"GreenOnly"` | Health gate policy for operator-managed rolling restart and upgrade pod restarts. GreenOnly preserves the existing restart health gate. GreenOrRecoverableYellow also allows yellow health when all primaries are active, no shard movement is in progress, and only replicas are unassigned. |
