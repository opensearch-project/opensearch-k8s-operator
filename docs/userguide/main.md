...
> **Note:** The init container queries the Kubernetes API for the node object, so the pods' `ServiceAccount` (`spec.general.serviceAccount`) must be allowed to `get` `nodes`. A minimal `ClusterRole` and `ClusterRoleBinding` granting this is shown in the [example manifest](../../opensearch-operator/examples/opensearch-zone-awareness.yaml). Node label values must not contain `,`, `{` or `}`.
